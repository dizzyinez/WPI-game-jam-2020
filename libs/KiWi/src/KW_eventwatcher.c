...
KW_Widget * CalculateMouseOver(KW_Widget * widget, int x, int y) {
        ...
        /* if not in composed geometry, nothing to see here then. rootwidget always
         * borked. */
        if (!(x > g.x && x < g.x + g.w && y > g.y && y < g.y + g.h) &&
            widget != widget->gui->rootwidget)
        {
                return NULL;
        }
...
void MouseMoved(KW_GUI * gui, int mousex, int mousey, int xrel, int yrel) {
        ...
        if (gui->cursordown == SDL_TRUE)
        {
                ...
                if (gui->currentdrag == NULL)
                {
                        ...
                        if (current != NULL)
                        {
                                KW_FireWidgetEvent(current, KW_OnDragStart, KW_ON_DRAGSTART,
                ...
                else
                {
                        ...
                        KW_FireWidgetEvent(current, KW_OnDrag, KW_ON_DRAG,
                                (current, mousex, mousey, xrel, yrel));
                }
...
void MousePressed(KW_GUI * gui, int mousex, int mousey, int button) {
        ...
        if (widget != NULL)
        {
                if (KW_IsWidgetInputEventsBlocked(widget) || KW_IsWidgetHidden(widget))
                        return;
                KW_FireWidgetEvent(widget, KW_OnMouseDown, KW_ON_MOUSEDOWN,
                        (widget, button));
...
void MouseReleased(KW_GUI * gui, int mousex, int mousey, int button) {
        ...
        if (widget &&
            (KW_IsWidgetInputEventsBlocked(widget) || KW_IsWidgetHidden(widget)))
                return;

        /* check if was under drag */
        if (gui->currentdrag != NULL)
        {
                KW_FireWidgetEvent(gui->currentdrag, KW_OnDragStop, KW_ON_DRAGSTOP,
                        (gui->currentdrag, mousex, mousey));
...
void TextInputReady(KW_GUI * gui, const char * text) {
        if (gui->currentfocus == NULL)
                return;
        KW_FireWidgetEvent(gui->currentfocus, KW_OnTextInput, KW_ON_TEXTINPUT,
                (gui->currentfocus, text));
}

void KeyUp(KW_GUI * gui, SDL_Keycode key, SDL_Scancode scan) {
        if (gui->currentfocus == NULL)
                return;
        KW_FireWidgetEvent(gui->currentfocus, KW_OnKeyUp, KW_ON_KEYUP,
                (gui->currentfocus, key, scan));
}

void KeyDown(KW_GUI * gui, SDL_Keycode key, SDL_Scancode scan) {
        if (gui->currentfocus == NULL)
                return;
        KW_FireWidgetEvent(gui->currentfocus, KW_OnKeyDown, KW_ON_KEYDOWN,
                (gui->currentfocus, key, scan));
}
...
void KW_ProcessEvents(KW_GUI * gui) {
        ...
        for (i = 0; i < gui->evqueuesize; i++)
        {
                ...
                case SDL_MOUSEMOTION:
                        MouseMoved(gui, event->motion.x, event->motion.y, event->motion.xrel,
                                event->motion.yrel);
                        break;
                ...
                case SDL_MOUSEBUTTONUP:
                        MouseReleased(gui, event->button.x, event->button.y,
                                event->button.button);
                        break;
